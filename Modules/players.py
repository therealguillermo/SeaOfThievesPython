...
from helpers import calculate_distance, object_to_screen, main_batch, \
     TEXT_OFFSET_X, TEXT_OFFSET_Y, OFFSETS, CONFIG
from Modules.display_object import DisplayObject

SHIP_COLOR = (255, 0, 0, 255)  # The color we want the indicator circle to be
FONT_SIZE = 12
CIRCLE_SIZE = 0  # The size of the indicator circle we want
COORD_OFFSET = 0x100
...
class Player(DisplayObject):
    ...
    def __init__(self, memory_reader, actor_id, address, my_coords, raw_name):
        ...
        player_name_location = self.rm.read_ptr(
            address + OFFSETS.get('APlayerState.PlayerName')
        )
        self.name = self.rm.read_name_string(player_name_location)
        ...
        if self.name == CONFIG.get('PlayerName'):
            self.to_delete = True
    ...
    def _build_text_render(self) -> Label:
        ...
        if self.screen_coords:
            return Label(self.text_str,
                         x=self.screen_coords[0], anchor_x='center',
                         y=self.screen_coords[1], anchor_y='center',
                         font_size=FONT_SIZE, color=SHIP_COLOR,
    ...
    def update(self, my_coords: dict):
        ...
        if self.screen_coords and self.distance < 350:
