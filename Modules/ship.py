...
SHIP_COLOR = (0, 0, 0)  # The color we want the indicator circle to be
...
class Ship(DisplayObject):
    ...
    def update(self, my_coords: dict):
        ...
        if self.screen_coords:
            ...
            # seamlessly at 1750m
            if "Near" in self.name and new_distance > 1750 and self.raw_name != "BP_AggressiveGhostShip_C":
                self.group.visible = False
            elif "Near" not in self.name and new_distance < 1750 and self.raw_name != "BP_AggressiveGhostShip_C":
                self.group.visible = False
            else:
