...
class ReadMemory:
    ...
    def read_CrewID(self, address: int):
        crewid = []
        for i in range(4):
            crewid.append(str(self.read_int(address + (0x0004*i))))
        return "/".join(crewid)

    def read_TArray(self, address: int):
        pointer = self.read_ptr(address)
        size = self.read_int(address + 8)
        maxsize = self.read_int(address + 8 + 4)
        return [pointer, size, maxsize]
    ...
    def read_name_string(self, address: int, byte: int = 32) -> str:
        ...
        :param address: address at whi
        ch to read a number of bytes
