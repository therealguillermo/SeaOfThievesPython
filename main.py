...
def update_all(_):
    ...
    """
    if DEBUG:
        smr.read_actors_debug()
    else:
        smr.set_config(menu.config)
        smr.read_actors()
        #print(smr.display_objects)
...
if __name__ == '__main__':
    ...
    player_count = Label("Player Count: {}",
                         x=SOT_WINDOW_W * 0.9,
                         y=SOT_WINDOW_H * 0.65, batch=main_batch)

    # The label for showing all players on the server under the count
    # pylint: disable=using-constant-test
    player_list = Label("\n".join(smr.server_players), x=SOT_WINDOW_W * 0.92,
                        y=(SOT_WINDOW_H - 25) * 0.65, batch=main_batch, width=300,
                        multiline=True)
